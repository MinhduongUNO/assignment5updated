...
@login_required
def event_emprecord(request, pk):
    record = get_object_or_404(Event, pk=pk)
    if request.method == "POST":
        # update
        form = EmpRecordForm(request.POST, instance=record)
        if form.is_valid():
            record = form.save(commit=False)
            record.save()
            record = EmpRecord.objects.filter()
            #return HttpResponseRedirect(reverse('event/event_list.html', {'records': record}))
            return render(request, 'event/event_list.html', {'records': record})
    else:
        form = EmpRecordForm(instance=record)
